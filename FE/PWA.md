...
Workbox is a tool used to cache both static and dynamic files or assets, There are many was in which you can use Workbox but the most generic one is through their CLI. You can use workbox CLI on any project just run `npm i --save-dev workbox-cli`, and then create a helper script.
...
You can now run `npm run generate-sw` to generate service worker file.

### Configuring Workbox Precaching

We can use `workbox-cli-config.js` to tell workbox what files we want to precache.
...
module.exports = {
  ...
  // where to output service worker file
  swDest: "public/service-worker.js",
};
```

Service worker file will have collection of file paths with revision number. Revision number is a file hash that is used by Workbox to tell if file changed. This hash is computed everytime we run our script to generate service worker.
So whenever you change something in your project you should run this command to recompute revision number of your precached files.

### Configuring Routing

Workbox routes are essentially endpoints to your server, cdn or some asset. You can configure workbox routing to set up dynamic chaching. Firstly we need to change our generate script to be `workbox inject:manifest`
...
Then we need to specify location of our custom service worker code in `swSrc` property, this is going to be combined with the code generated by Workbox.
...
First we want to register route or endpoint we want to intercept, and declare or implement caching strategy. To accomplish this task we can use `registerRoute` from `WorkboxSW`.
This function takes two arguments:

1. route or endpoint as string or regex
2. caching strategy or interception handler

Workbox offers some caching strategies out of the box like `staleWhileRevalidate` which will reach into cache and send a request, if request returns successfully, cache is going to be updated. This means that whenever we issue a request FE is going to intialy have old/stale value until request resolves.
Example below uses regex to intercept any request sent to google fonts or icons.
...
Example of using string as route

```js
workboxSW.router.registerRoute(
  "https://cdnjs.cloudflare.com/ajax/meterial.min.css",
  workboxSW.strategies.staleWhileRevalidate({
...
When there are no strategies that fit our need we can create custom one, here we are intercepting requests sent to our backend and saving the results in IndexedDB

```js
workboxSW.router.registerRoute(
  "https://pwgram-99adf.firebaseio.com/posts.json",
  function (args) {
    return fetch(args.event.request).then((res) => {
      const clone = res.clone();
      clearAllData("posts")
        .then(() => clone.json())
        .then((posts) => writeData("posts", posts));
      return res;
    });
  }
);
...
First argument doesn't have to be string or regex it can also be a function that takes `routeData` as argument and returns `true`. We can now use this to provide and offline HTML fallback, by checking whether user wants to download/GET html.

```js
function updateDynamicCache(request, response) {
  ...
  function (args) {
    return caches.match(args.event.request).then((res) => {
      ...
      return fetch(args.event.request)
        .then((res) => {
          updateDynamicCache(args.event.request, res.clone());
          return res;
        })
...
### Handling Background Sync and Push Notification

Workbox doesn't provide helpers around this, at least not now, but you can move your service worker code in `service-worker-base.js` file. Which goes to show that you can work with this file in the same way you would with your own custom service worker file.
